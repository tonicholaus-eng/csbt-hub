...
export default function WishlistWatchlist() {
  ...
  return <div className="grid gap-6 xl:grid-cols-2">
    ...
    <Surface as="section" className="p-5">
      ...
      <div className="mt-5 space-y-2">{watches.length ? watches.map((row) => <div key={row.id} className="rounded-2xl border border-slate-200 p-3 dark:border-white/10"><div className="flex items-center justify-between gap-3"><Link href={`/values/${encodeURIComponent(row.item_id)}`} className="truncate text-sm font-black text-slate-900 dark:text-white">{row.item_name}</Link><button type="button" onClick={async()=>{await supabase.from("value_watchlist").delete().eq("id",row.id); void reload();}} className="min-h-11 rounded-xl px-3 text-xs font-black text-rose-500">Remove</button></div><div className="mt-3 grid grid-cols-3 gap-2"><span className="rounded-xl bg-slate-100 px-2 py-2 text-center text-[10px] font-black dark:bg-white/5">{row.source === "GCASH" ? "💸 GCash" : "🦈 Elve"}</span><span className="rounded-xl bg-slate-100 px-2 py-2 text-center text-[10px] font-black dark:bg-white/5">{row.value_type}</span><select value={row.alert_percent} onChange={async(e)=>{await supabase.from("value_watchlist").update({alert_percent:Number(e.target.value)}).eq("id",row.id); void reload();}} className="min-h-11 rounded-xl border border-slate-200 bg-white px-2 text-[10px] font-black dark:border-white/10 dark:bg-slate-900">{[5,10,15,20,25].map(n=><option key={n} value={n}>{n}% change</option>)}</select></div></div>) : <EmptyState icon="↗" title="No value alerts yet" description="Open an item on Values and add it to your watchlist to track meaningful changes." href="/values" actionLabel="Browse values" />}</div>
    </Surface>
