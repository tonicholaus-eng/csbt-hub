...
export default function WishlistWatchlist() {
  ...
  return <div className="grid gap-6 xl:grid-cols-2 xl:gap-8">
    <Surface as="section" className="csbt-feature-panel csbt-panel-accent-wishlist p-5 sm:p-6 lg:p-9">
      ...
      <div className="mt-5 space-y-2">{wishlist.length ? wishlist.map((row) => { const item=getItemById(row.item_id); const d=getItemCategoryDetails(item?.CATEGORY ?? row.category); return <div key={row.id} className="csbt-list-row flex items-center gap-3 rounded-2xl p-3 lg:p-4"><span className="flex h-12 w-12 items-center justify-center rounded-xl bg-[var(--surface-3)] ring-1 ring-[var(--border)]">{row.image_url ? <Image src={row.image_url} alt="" width={48} height={48} className="h-10 w-10 object-contain" /> : d.icon}</span><span className="min-w-0 flex-1"><Link href={`/values/${encodeURIComponent(row.item_id)}`} className="block truncate text-sm font-black text-[var(--foreground)]">{row.item_name}</Link><span className="text-xs font-bold text-slate-400">{d.label}</span></span><Link href={`/exchange?tab=browse&q=${encodeURIComponent(row.item_name)}`} className="inline-flex min-h-11 items-center rounded-xl border border-[var(--border)] px-3 text-xs font-black">Find trades</Link><button type="button" aria-label={`Remove ${row.item_name} from wishlist`} onClick={async()=>{await supabase.from("wishlist_items").delete().eq("id",row.id); void reload();}} className="h-11 w-11 rounded-xl bg-rose-50 font-black text-rose-600 dark:bg-rose-500/10">×</button></div>; }) : <EmptyState icon="♡" title="Your wishlist is empty" description="Add items you want and CSBT can use them for matching, offers, and future trade planning." />}</div>
    </Surface>
    ...
    <Surface as="section" className="csbt-feature-panel csbt-panel-accent-alerts p-5 sm:p-6 lg:p-9">
      ...
      <div className="mt-5 space-y-2">{watches.length ? watches.map((row) => <div key={row.id} className="csbt-list-row rounded-2xl p-3 lg:p-4"><div className="flex items-center justify-between gap-3"><Link href={`/values/${encodeURIComponent(row.item_id)}`} className="truncate text-sm font-black text-[var(--foreground)]">{row.item_name}</Link><button type="button" onClick={async()=>{await supabase.from("value_watchlist").delete().eq("id",row.id); void reload();}} className="min-h-11 rounded-xl px-3 text-xs font-black text-rose-500">Remove</button></div><div className="mt-3 grid grid-cols-3 gap-2"><span className="rounded-xl bg-[var(--surface-3)] px-2 py-2 text-center text-xs font-black ring-1 ring-[var(--border)]">{row.source === "GCASH" ? "💸 GCash" : "🦈 Elve"}</span><span className="rounded-xl bg-[var(--surface-3)] px-2 py-2 text-center text-xs font-black ring-1 ring-[var(--border)]">{row.value_type}</span><select value={row.alert_percent} onChange={async(e)=>{await supabase.from("value_watchlist").update({alert_percent:Number(e.target.value)}).eq("id",row.id); void reload();}} className="csbt-input-field min-h-11 rounded-xl px-2 text-xs font-black">{[5,10,15,20,25].map(n=><option key={n} value={n}>{n}% change</option>)}</select></div></div>) : <EmptyState icon="↗" title="No value alerts yet" description="Open an item on Values and add it to your watchlist to track meaningful changes." href="/values" actionLabel="Browse values" />}</div>
    </Surface>
    {error && <p className="xl:col-span-2 rounded-xl bg-rose-50 p-3 text-xs font-bold text-rose-700 dark:bg-rose-500/10 dark:text-rose-300">{error.includes("relation") ? "Wishlist services are temporarily unavailable. Please try again later." : error}</p>}
  </div>;
}
