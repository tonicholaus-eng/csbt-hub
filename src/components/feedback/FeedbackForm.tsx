...
export default function FeedbackForm(){
  ...
  return <section className="mx-auto max-w-3xl rounded-[var(--radius-section)] bg-[var(--surface-1)] p-5 shadow-[var(--shadow-sm)] ring-1 ring-[var(--border)] sm:p-7"><div className="grid gap-5"><label><span className="mb-2 block text-xs font-black text-slate-700 dark:text-slate-300">Feedback type</span><select value={category} onChange={e=>{setCategory(e.target.value as typeof category);setItem(null)}} className="min-h-12 w-full rounded-[var(--radius-control)] border border-[var(--border-strong)] bg-[var(--surface-3)] px-4 text-sm font-black">{categories.map(([v,l])=><option key={v} value={v}>{l}</option>)}</select></label>{itemRelevant&&<div><span className="mb-2 block text-xs font-black text-slate-700 dark:text-slate-300">Related item {category==="MISSING_ITEM"?"(optional if it isn't in the database yet)":""}</span><ItemSearchPicker onSelect={setItem} placeholder="Search the item this feedback is about…"/>{item&&<p className="mt-2 rounded-xl bg-amber-50 px-3 py-2 text-xs font-black text-amber-800 dark:bg-amber-400/10 dark:text-amber-200">Selected: {item.NAME} <button onClick={()=>setItem(null)} className="ml-2 underline">clear</button></p>}</div>}<label><span className="mb-2 block text-xs font-black text-slate-700 dark:text-slate-300">Tell us what happened or what you'd like added</span><textarea value={message} onChange={e=>setMessage(e.target.value)} maxLength={2000} rows={7} placeholder={category==="WRONG_VALUE"?"Example: Frost Dragon GCash value looks outdated because…":category==="BUG"?"What page were you on, what did you tap, and what happened?":"Describe your suggestion as clearly as possible…"} className="w-full resize-y rounded-[var(--radius-control)] border border-[var(--border-strong)] bg-[var(--surface-3)] px-4 py-3 text-sm outline-none focus:border-[var(--gold)]"/><span className="mt-1 block text-right text-[10px] font-bold text-slate-400">{message.length}/2000</span></label>{error&&<p className="rounded-xl bg-rose-50 p-3 text-xs font-bold text-rose-700 dark:bg-rose-500/10 dark:text-rose-300">{error.includes("relation")?"Run src/lib/supabase/phase-two.sql in Supabase to enable feedback.":error}</p>}{notice&&<p className="rounded-xl bg-emerald-50 p-3 text-xs font-bold text-emerald-700 dark:bg-emerald-500/10 dark:text-emerald-300">{notice}</p>}<div className="flex flex-wrap items-center justify-between gap-3"><p className="text-xs text-slate-400">{user?"Your signed-in CSBT account will be attached to the submission.":"You can submit without signing in."}</p><button disabled={busy||message.trim().length<5} onClick={()=>void submit()} className="csbt-btn-primary min-h-12 px-6 disabled:opacity-40">{busy?"Sending…":"Send Feedback"}</button></div></div></section>;
}
