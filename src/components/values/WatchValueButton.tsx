...
export default function WatchValueButton({ itemId, itemName, source, valueType }: { itemId: string; itemName: string; source: ValueSource; valueType: ValueType }) {
  ...
  return <div className="mt-3 flex flex-wrap items-center gap-2"><select value={threshold} onChange={(e)=>setThreshold(Number(e.target.value))} className="min-h-11 rounded-2xl border border-slate-200 bg-white px-3 text-xs font-black dark:border-white/10 dark:bg-slate-900">{[5,10,15,20,25].map(n=><option key={n} value={n}>{n}% change</option>)}</select><button type="button" disabled={busy} onClick={()=>void (watchId?saveWatch():saveWatch())} className={`min-h-11 rounded-2xl border px-4 text-xs font-black disabled:opacity-50 ${watchId?"border-emerald-200 bg-emerald-50 text-emerald-700 dark:border-emerald-400/20 dark:bg-emerald-400/10 dark:text-emerald-300":"border-amber-200 bg-amber-50 text-amber-800 dark:border-amber-400/20 dark:bg-amber-400/10 dark:text-amber-200"}`}>{busy?"Saving…":watchId?`✓ Watching ${threshold}%+ changes`:`🔔 Watch ${threshold}%+ changes`}</button>{watchId&&<button type="button" disabled={busy} onClick={()=>void removeWatch()} className="min-h-11 rounded-2xl px-3 text-xs font-black text-rose-500">Remove</button>}{error&&<p className="w-full text-xs font-bold text-rose-600 dark:text-rose-300">{error.includes("relation")?"Apply the CSBT Supabase migration first.":error}</p>}</div>;
}
