...
export default function TradeRoomExperience({
  ...
  const [inventoryApplyState, setInventoryApplyState] = useState<"IDLE" | "SAVING" | "APPLIED">("IDLE");
  const [inventoryApplyNotice, setInventoryApplyNotice] = useState<string | null>(null);
  ...
  const source: CSBTValueSource = room?.lock_snapshot.value_source ?? "GCASH";
  ...
  async function applyCompletedTradeToInventory() {
    const client = supabase;
    if (!client || !user || room?.game_id !== "adopt-me" || room?.status !== "COMPLETED" || !isParticipant || inventoryApplyState === "SAVING") return;
    setInventoryApplyState("SAVING");
    setInventoryApplyNotice(null);
    const { data, error: applyError } = await client.rpc("marketplace_apply_completed_trade_to_inventory", { p_room_id: roomId });
    if (applyError) {
      setInventoryApplyState("IDLE");
      setError("Could not update your saved inventory. Your inventory was not changed.");
      return;
    }
    setInventoryApplyState("APPLIED");
    setInventoryApplyNotice(
      data === "ALREADY_APPLIED"
        ? "This completed trade was already applied to your saved inventory."
        : "Saved inventory updated from the locked completed trade."
    );
    await refreshRoom();
  }
  ...
  return (
    <div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_330px]">
      <div className="space-y-5">
        ...
        {isStaffViewer ? <section className="rounded-[28px] border border-rose-100 bg-rose-50/75 p-5 dark:border-rose-400/10 dark:bg-rose-400/[0.045]"><p className="text-[10px] font-black uppercase tracking-[0.16em] text-rose-600">Moderation review</p><h2 className="mt-1 text-xl font-black">Read-only room evidence</h2><p className="mt-3 text-xs font-semibold leading-5 text-slate-600 dark:text-slate-300">You can inspect the locked offer, message history, and timeline for a submitted report. Moderators cannot change trade status or send room messages.</p><Link href={`${exchangeBasePath}/moderation`} className="mt-3 inline-flex rounded-xl bg-rose-500 px-4 py-2.5 text-xs font-black text-white">Return to Moderation Desk</Link></section> : isAssignedMiddleman ? <section className="rounded-[28px] border border-amber-100 bg-amber-50/75 p-5 dark:border-amber-400/10 dark:bg-amber-400/[0.045]"><div className="flex flex-wrap items-center justify-between gap-3"><div><p className="text-[10px] font-black uppercase tracking-[0.16em] text-amber-600">Middleman case</p><h2 className="mt-1 text-xl font-black">Protect the locked agreement</h2></div><span className="rounded-full bg-white px-3 py-1.5 text-xs font-black dark:bg-white/5">{mmRequest?.status?.replaceAll("_", " ") ?? "ASSIGNED"}</span></div><p className="mt-3 text-xs font-semibold leading-5 text-slate-600 dark:text-slate-300">You can see the locked items and participate in the structured room chat, but only the two traders can change transaction status or confirm completion.</p><Link href={`${exchangeBasePath}/middleman`} className="mt-3 inline-flex rounded-xl bg-amber-400 px-4 py-2.5 text-xs font-black text-white">Return to Middleman Desk</Link></section> : <section className="rounded-[28px] border border-white/70 bg-white/82 p-5 dark:border-white/10 dark:bg-slate-950/65"><div className="flex flex-wrap items-center justify-between gap-3"><div><p className="text-[10px] font-black uppercase tracking-[0.16em] text-cyan-600">Transaction status</p><h2 className="mt-1 text-xl font-black">Complete the trade step by step</h2></div><span className="rounded-full bg-slate-100 px-3 py-1.5 text-xs font-black dark:bg-white/5">{room.status.replaceAll("_", " ")}</span></div><div className="mt-4 grid gap-2 sm:grid-cols-3"><button onClick={() => void setStatus("CONNECTING")} disabled={room.status === "COMPLETED"} className="rounded-2xl border border-slate-200 bg-white p-3 text-xs font-black disabled:opacity-40 dark:border-white/10 dark:bg-white/5">1. Adding / connecting</button><button onClick={() => void setStatus("JOINED")} disabled={room.status === "COMPLETED"} className="rounded-2xl border border-slate-200 bg-white p-3 text-xs font-black disabled:opacity-40 dark:border-white/10 dark:bg-white/5">2. Joined server</button><button onClick={() => void confirmCompletion()} disabled={room.status === "COMPLETED" || myConfirmed} className="rounded-2xl bg-emerald-500 p-3 text-xs font-black text-white disabled:opacity-40">{myConfirmed ? "✓ You confirmed" : "3. Confirm completed"}</button></div>{myConfirmed && !otherConfirmed && <p className="mt-3 text-xs font-bold text-amber-600">Waiting for the other trader to confirm completion.</p>}{room.status === "COMPLETED" && room.game_id === "adopt-me" && <div className="mt-3 grid gap-3 rounded-[18px] border border-emerald-500/20 bg-emerald-500/5 p-3 sm:grid-cols-[1fr_auto] sm:items-center"><div><p className="text-sm font-black text-emerald-700 dark:text-emerald-300">Update your saved inventory</p><p className="mt-1 text-xs font-semibold text-slate-500 dark:text-slate-400">Uses the locked completed trade: removes your outgoing items and adds your incoming items only after you confirm here.</p>{inventoryApplyNotice && <p className="mt-2 text-xs font-bold text-emerald-700 dark:text-emerald-300" aria-live="polite">{inventoryApplyNotice}</p>}</div><button onClick={() => void applyCompletedTradeToInventory()} disabled={inventoryApplyState !== "IDLE"} className="rounded-2xl bg-emerald-500 px-4 py-2.5 text-xs font-black text-white disabled:cursor-not-allowed disabled:opacity-60">{inventoryApplyState === "SAVING" ? "Updating…" : inventoryApplyState === "APPLIED" ? "✓ Inventory updated" : "Update saved inventory"}</button></div>}{room.status === "COMPLETED" && <button onClick={() => setReviewOpen(true)} className="mt-3 rounded-2xl bg-amber-400 px-4 py-2.5 text-xs font-black text-white">Leave a review</button>}</section>}
      ...
      <AccessibleDialog open={reportOpen} onClose={() => setReportOpen(false)} title="Report a problem in this room" className="max-w-lg"><div className="rounded-[var(--radius-section)] bg-white p-5 dark:bg-slate-950"><p className="text-[10px] font-black uppercase tracking-[0.16em] text-rose-500">Exchange safety report</p><h2 className="mt-1 text-xl font-black">Report a problem in this room</h2><select value={reportCategory} onChange={(event) => setReportCategory(event.target.value)} className="mt-4 min-h-11 w-full rounded-2xl border border-slate-200 bg-white px-3 text-sm font-black dark:border-white/10 dark:bg-slate-900"><option value="SWITCH_ATTEMPT">Switch attempt / changed offer</option><option value="SCAM_RISK">Scam risk</option><option value="OFF_PLATFORM_LINK">Off-platform contact/link</option><option value="HARASSMENT">Harassment</option><option value="SPAM">Spam</option><option value="OTHER">Other</option></select><textarea value={reportDetails} onChange={(event) => setReportDetails(event.target.value)} rows={4} maxLength={1500} placeholder="Explain what happened so CSBT staff can review the locked room and timeline…" className="mt-3 w-full rounded-2xl border border-slate-200 p-3 text-sm dark:border-white/10 dark:bg-slate-900"/><div className="mt-3 flex justify-end gap-2"><button onClick={() => setReportOpen(false)} className="rounded-xl border border-slate-200 px-4 py-2 text-xs font-black">Cancel</button><button disabled={reportDetails.trim().length < 5} onClick={() => void reportProblem()} className="rounded-xl bg-rose-500 px-4 py-2 text-xs font-black text-white disabled:opacity-40">Submit & dispute room</button></div></div></AccessibleDialog>
      <AccessibleDialog open={reviewOpen} onClose={() => setReviewOpen(false)} title="Rate this trader" className="max-w-lg"><div className="rounded-[var(--radius-section)] bg-white p-5 dark:bg-slate-950"><h2 className="text-xl font-black">Rate this trader</h2><div className="mt-4 flex gap-2">{[1,2,3,4,5].map((star) => <button key={star} onClick={() => setRating(star)} className={`text-3xl ${star <= rating ? "opacity-100" : "opacity-25"}`}>⭐</button>)}</div><textarea value={reviewComment} onChange={(event) => setReviewComment(event.target.value)} rows={3} maxLength={500} placeholder="Optional review…" className="mt-4 w-full rounded-2xl border border-slate-200 p-3 text-sm dark:border-white/10 dark:bg-slate-900"/><div className="mt-3 flex justify-end gap-2"><button onClick={() => setReviewOpen(false)} className="rounded-xl border border-slate-200 px-4 py-2 text-xs font-black">Cancel</button><button onClick={() => void submitReview()} className="rounded-xl bg-amber-400 px-4 py-2 text-xs font-black text-white">Submit review</button></div></div></AccessibleDialog>
    </div>
  );
}

function LockedSide({ title, items, source }: { title: string; items: ExchangeItem[]; source: CSBTValueSource }) { const total = items.reduce((sum,item)=>sum+(item.snapshot_value??0)*item.quantity,0); return <div className="rounded-[24px] border border-white/70 bg-white/75 p-4 dark:border-white/10 dark:bg-white/[0.035]"><div className="flex justify-between"><h3 className="text-sm font-black">{title}</h3><span className="text-xs font-black text-slate-400">{sourceSymbol(source)}{formatTradeValue(total)}</span></div><div className="mt-3 space-y-2">{items.map((item,index)=><div key={`${item.item_id}-${index}`} className="flex items-center gap-2"><span className="flex h-10 w-10 items-center justify-center overflow-hidden rounded-xl bg-slate-100 dark:bg-white/5">{item.image_url?<Image src={item.image_url} alt="" width={40} height={40} unoptimized className="h-9 w-9 object-contain"/>:"📦"}</span><span className="min-w-0 flex-1 truncate text-xs font-black">{item.quantity>1?`${item.quantity}× `:""}{item.value_type!=="NORMAL"?`${item.value_type} `:""}{item.item_name}</span></div>)}</div></div>; }
