...
export default function ListingDetail({ listingId }: { listingId: string }) {
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      const [{ data: trustRows }, { data: categoryRows }, { data: itemRows }, { data: eventRows }] = await Promise.all([
        ...
        client.from("marketplace_events").select("event_type,listing_id,item_id").in("item_id", haveIds.length ? haveIds : ["__none__"]).limit(500),
      ]);
      if (!active) return;
    ...
    return () => { active = false; };
  }, [authLoading, listingId, supabase, user]);
  ...
  useEffect(() => {
    ...
    void fetch("/api/exchange/event", {
      ...
      body: JSON.stringify({ eventType: "LISTING_VIEW", listingId: listing.id, itemId: listing.items.find((item) => item.side === "HAVE")?.item_id, valueSource: listing.value_source }),
    }).catch(() => undefined);
  }, [listing]);

  const match = useMemo(() => listing ? scoreListingMatch(listing, inventory, wishlistIds, preferences) : null, [inventory, listing, preferences, wishlistIds]);
  ...
  async function blockTrader() {
    ...
    if (blockError) setError(blockError.message); else window.location.href = "/exchange";
  }
      ...
      <div className="space-y-5">
        <section className="rounded-[32px] border border-white/70 bg-white/85 p-5 shadow-[0_20px_60px_rgba(15,23,42,.1)] dark:border-white/10 dark:bg-slate-950/70 sm:p-6">
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div><p className="text-[10px] font-black uppercase tracking-[0.16em] text-amber-500">CSBT Exchange listing</p><h1 className="mt-2 text-3xl font-black text-slate-950 dark:text-white">{listing.title || `${have[0]?.item_name ?? "Trade"} offer`}</h1><p className="mt-2 text-sm font-semibold text-slate-500">{listing.display_name} • {listing.intent.replaceAll("_", " ")} • {listing.value_source}</p></div>
