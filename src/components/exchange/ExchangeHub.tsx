        ...
        {gameId === "adopt-me" ? (
          ...
          <>
            <section className="rounded-[var(--radius-card)] border border-red-400/20 bg-red-500/[0.045] p-4 lg:p-5">
              ...
              <h3 className="mt-1 text-lg font-black text-slate-950 dark:text-white">Trade MM2 weapons here too</h3>
              <p className="mt-2 text-xs font-semibold leading-5 text-slate-500">All {adapter.items.length.toLocaleString()} MM2 weapons can go in a listing or an offer. Adopt Me items stay on the Adopt Me side.</p>
...
function MarketIntelligence({ gameId, topMarket, events, listings }: { gameId: "adopt-me" | "mm2"; topMarket: Array<{ itemId: string; name: string; source: CSBTValueSource; available: number; wanted: number; accepted: number; acceptedValue: number; acceptedValueCount: number }>; events: ExchangeMarketEvent[]; listings: ExchangeListing[] }) {
  ...
  return <section className="mt-5"><SectionHeading eyebrow="CSBT-owned market data" title="Market Intelligence" /><p className="mt-2 max-w-3xl text-sm font-semibold text-slate-500">Exchange turns listings, searches, views, offers, and completed trades into CSBT-owned market signals. As activity grows, this becomes a real picture of supply, demand, velocity, and accepted values instead of relying only on external value lists.</p><div className="mt-4 grid grid-cols-2 gap-2 sm:grid-cols-3 lg:grid-cols-6"><Metric value={String(listings.length)} label="Open listings" /><Metric value={String(accepted)} label="Accepted offers" /><Metric value={String(completed)} label="Completed rooms" /><Metric value={String(views24h)} label="Views · 24h" /><Metric value={String(searches24h)} label="Searches · 24h" /><Metric value={String(offers24h)} label="Offers · 24h" /></div>{topQueries.length > 0 && <div className="mt-4 rounded-[24px] border border-violet-100 bg-violet-50/70 p-4 dark:border-violet-400/10 dark:bg-violet-400/[0.04]"><p className="text-[10px] font-black uppercase tracking-[0.14em] text-violet-600 dark:text-violet-300">What traders are searching now</p><div className="mt-3 flex flex-wrap gap-2">{topQueries.map(([query,count]) => <span key={query} className="rounded-full border border-violet-100 bg-white px-3 py-1.5 text-[10px] font-black text-violet-700 dark:border-violet-400/10 dark:bg-white/5 dark:text-violet-200">{query} <span className="opacity-50">×{count}</span></span>)}</div></div>}<div className="mt-5 hidden overflow-x-auto rounded-[26px] border border-white/70 bg-white/80 dark:border-white/10 dark:bg-slate-950/65 sm:block"><div className="min-w-[650px]"><div className="grid grid-cols-[minmax(160px,1fr)_70px_70px_70px_70px_110px] border-b border-slate-200 px-4 py-3 text-[9px] font-black uppercase tracking-[0.1em] text-slate-400 dark:border-white/10"><span>Item</span><span>Source</span><span>Supply</span><span>Wanted</span><span>Done</span><span>CSBT Market</span></div>{topMarket.map((row) => <Link href={marketAdapter.itemProfileHref(marketAdapter.getItem(row.itemId) ?? marketAdapter.items[0])} key={`${row.itemId}-${row.source}`} className="grid grid-cols-[minmax(160px,1fr)_70px_70px_70px_70px_110px] items-center border-b border-slate-100 px-4 py-3 text-xs last:border-0 dark:border-white/5"><span className="truncate font-black text-slate-800 dark:text-white">{row.name}</span><span className="font-black text-slate-500">{sourceLabel(row.source)}</span><span className="font-bold text-slate-500">{row.available}</span><span className="font-bold text-violet-600">{row.wanted}</span><span className="font-black text-emerald-600">{row.accepted}</span><span className="font-black text-cyan-600">{row.acceptedValueCount ? `${sourceSymbol(row.source)}${(row.acceptedValue / row.acceptedValueCount).toLocaleString(undefined,{maximumFractionDigits:2})}` : "Collecting"}</span></Link>)}</div></div><div className="mt-4 grid gap-2 sm:hidden">{topMarket.map((row) => <Link href={marketAdapter.itemProfileHref(marketAdapter.getItem(row.itemId) ?? marketAdapter.items[0])} key={`m-${row.itemId}-${row.source}`} className="rounded-[18px] border border-white/70 bg-white/80 p-3.5 dark:border-white/10 dark:bg-slate-950/65"><div className="flex items-baseline justify-between gap-3"><span className="min-w-0 flex-1 truncate text-sm font-black text-slate-800 dark:text-white">{row.name}</span><span className="shrink-0 text-[11px] font-black uppercase tracking-[0.08em] text-slate-400">{sourceLabel(row.source)}</span></div><div className="mt-2.5 grid grid-cols-2 gap-x-3 gap-y-2"><span className="flex items-baseline justify-between gap-2 text-[12px] font-bold text-slate-500"><span>Supply</span><b className="font-black text-slate-700 dark:text-slate-200">{row.available}</b></span><span className="flex items-baseline justify-between gap-2 text-[12px] font-bold text-slate-500"><span>Wanted</span><b className="font-black text-violet-600">{row.wanted}</b></span><span className="flex items-baseline justify-between gap-2 text-[12px] font-bold text-slate-500"><span>Done</span><b className="font-black text-emerald-600">{row.accepted}</b></span><span className="flex items-baseline justify-between gap-2 text-[12px] font-bold text-slate-500"><span>Market</span><b className="font-black text-cyan-600">{row.acceptedValueCount ? `${sourceSymbol(row.source)}${(row.acceptedValue / row.acceptedValueCount).toLocaleString(undefined,{maximumFractionDigits:2})}` : "Collecting"}</b></span></div></Link>)}</div></section>;
}
